...
class GuestViewSet(GenericViewSet, ListModelMixin, CreateModelMixin):
    ...
    def create(self, request, *args, **kwargs):
        ...
        valid_email=email_serializer.validated_data['email']
        guest = get_object_or_404(User, email=valid_email)
        if guest == request.user:
            raise ConflictError("You can't be your own guest")
        board.guests.add(guest)
        send_mail(subject='Guest',
                  from_email=None,
                  message='Added as guest in board '+board.name+' by '+board.owner.username,
...
class ColumnViewSet(ModelViewSet):
    ...
    def perform_create(self, serializer):
        serializer.save(board=get_object_or_404(Board, id=self.kwargs.get('board_pk')))


class UserBoards(ListAPIView):
    serializer_class = SimpleBoardSerializer
    def get_queryset(self):
        return get_object_or_404(User,id=self.kwargs.get('id')).boards.all()


class UserGuestedBoards(ListAPIView):
    serializer_class = SimpleBoardSerializer
    def get_queryset(self):
        return get_object_or_404(User,id=self.kwargs.get('id')).guested_boards.all()
...
class NoteViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        return get_object_or_404(Column,id=self.kwargs.get('column_pk')).notes.all()
